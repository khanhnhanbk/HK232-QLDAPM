...
export class OrganizationTreeComponent extends AppComponentBase implements OnInit {
  ...
  constructor(
    ...
    private _classtimeService: ClasstimeServiceProxy
  ) {
    super(injector);
  }
  ...
  nodeSelect(event) {
    ...
    this._classtimeService.getClasstimeById(event.node.data.id).subscribe((result) => {
      this.selectedOuTime = result;
    });
  ...
  saveTime() {
    this._classtimeService.updateClasstime(
      new UpdateClasstimeInput({
        organizationUnitId: this.selectedOu.data.id, // this is the id of the organization unit that is select
        startTime: this.selectedOuTime.startTime,
        endTime: this.selectedOuTime.endTime,
      })
    ).subscribe(() => {
